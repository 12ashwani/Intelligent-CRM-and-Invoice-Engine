...
# LEAVE MANAGEMENT
# =========================================================
@hr_bp.route("/leave-requests", endpoint="leave_requests")
@login_required
...
@hr_bp.route("/leave-requests/<int:leave_id>/approve", methods=["POST"], endpoint="approve_leave")
@login_required
def approve_leave(leave_id):
    ...
    try:
        ...
        flash(f"Failed to approve leave request: {error}", "danger")

    return redirect(url_for("hr.leave_requests"))


@hr_bp.route("/leave-requests/<int:leave_id>/reject", methods=["POST"], endpoint="reject_leave")
@login_required
def reject_leave(leave_id):
    ...
    try:
        ...
        flash(f"Failed to reject leave request: {error}", "danger")
